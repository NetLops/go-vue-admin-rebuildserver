...
// return: total int64
func (apiServer *ApiService) GetAPIInfoList(api system.SysApi, info request.PageInfo, order string, desc bool) (err error, list interface{}, total int64) {
	limit := info.PageSize
...
func (apiService *ApiService) DeleteApiByIds(ids []string) (err error) {
	err = global.GVA_DB.Delete(&[]system.SysApi{}, "id in ?", ids).Error
	return err
}
