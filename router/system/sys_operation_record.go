...
func (s *OperationRecordRouter) InitSysOperationRecordRouter(Router *gin.RouterGroup) {
	...
	authorityMenuApi := v1.ApiGroupApp.SystemApiGroup.OperationRecordApi
	{
		...
		operationRecordRouter.GET("findSysOpeerationRecord", authorityMenuApi.FindSysOperationRecord)                 // 根据ID获取SysOperationRecord
		operationRecordRouter.GET("getSysOperationRecordList", authorityMenuApi.GetSysOperationRecordList)            // 根据SysOperationRecord 列表
	}
}
