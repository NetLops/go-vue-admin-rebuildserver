...
func (autoApi *AutoCodeApi) CreateTemp(c *gin.Context) {
	...
	if a.AutoCreateApiToSql {
		if ids, err := autoCodeService.AutoCreateApi(&a); err != nil {
			global.GVA_LOG.Error("自动化创建失败!请自行晴空垃圾数据!", zap.Error(err))
	...
	if err != nil {
		...
		c.Writer.Header().Add("Content-Dispotion", fmt.Sprintf("attachment; filename=%s", "ginvueadmin.zip")) // fmt.Sprintf("attachment; filename=%s", filename)对下载的文件重命名
...
// @accept application/json
// @Produce application/json
// @Success 200 {object} response.Response{data=map[string]interface{},msg=string} "获取当前所有数据"
...
// @accept application/json
// @Produce application/json
// @Produce 200 {object} response.Response{data=map[string]interface{}, msg=string} "获取当前数据库所有表"
