...
func (b *BaseApi) tokenNext(c *gin.Context, user system.SysUser) {
	...
	if err, jwtStr := jwtService.GetRedisJWT(user.Username); err == redis.Nil {
		...
		global.GVA_LOG.Error("设置登录状态！", zap.Error(err))
...
// @Security ApiKeyAuth
// @Produce application/json
// @Param data body systemReq.ChangePasswordStruct trur "用户名，原密码，新密码"
...
func (b *BaseApi) ChangePassword(c *gin.Context) {
	...
	if err, _ := userService.ChangePassword(u, user.NewPassword); err != nil {
		global.GVA_LOG.Error("修改失败，原密码与当前账户不符", zap.Error(err))
		response.FailWIthMessage(err.Error(), c)
	} else {
...
// @Success 200 {object} response.Response{data=response.PageResult,msg=string} "分页获取用户列表，返回包括列表，总数，页码，每页数据"
...
// @Produce application/json
// @Param data body system.SysUser true "ID, 用户名, 昵称, 头像链接"
// @Succes 200 {object} response.Response{data=map[string]interface{},msg=string} "设置用户信息"
